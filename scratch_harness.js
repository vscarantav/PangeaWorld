...
const document = {
    ...
    addEventListener: () => {}
        ...
        const TERRAIN = {
            ...
            SOLHAVEN: { color: '#f1c40f', name: 'Solhaven' },
            KORVATH: { color: '#16a085', name: 'Korvath' },
            VALDORIA: { color: '#f39c12', name: 'Valdoria (Desert)' },
            LUNARA: { color: '#d35400', name: 'Lunara' },
            NORDVIK: { color: '#aed6f1', name: 'Nordvik (Tundra)' },
            ZEPHYRIA: { color: '#228b22', name: 'Zephyria' },
            DRAKMOOR: { color: '#7f8c8d', name: 'Drakmoor (Mountains)' },
            CITY: { color: '#ffffff', name: 'Company City' } // Base fallback
        };
        ...
        function initGrid() {
            ...
            for(let i = 0; i < 2000; i++) {
                ...
                let targetCountry = candidates.length > 0 ? candidates[Math.floor(Math.random() * candidates.length)] : null;
                ...
                if (targetCountry) {
                    ...
                    if (countryTris.length > 0) {
                        startTri = countryTris[Math.floor(Math.random() * countryTris.length)];
                    }
                }
                
                if (!startTri) {
                    startTri = triangles[Math.floor(Math.random() * triangles.length)];
                }
                ...
                while(currentTri && length < maxLength) {
                    ...
                    if (originalTerrain !== TERRAIN.RIVER) {
                        ...
                        let countryDef = countries.find(c => c.terrain === originalTerrain);
                        if (countryDef) {
            ...
            mountains.sort(() => Math.random() - 0.5); // Shuffle for random maze generation
            ...
            mountains.forEach(m => {
                ...
                if (passibleEdges.length === 0) {
                    ...
                    if (validEdges.length > 0) {
                        edgeToMakePassible = validEdges[Math.floor(Math.random() * validEdges.length)];
                    } else {
                        edgeToMakePassible = mEdges[Math.floor(Math.random() * mEdges.length)];
                    }
            ...
            countries.forEach((country) => {
                ...
                let requiredPortCount = (country.id === 'NORDVIK' || country.id === 'LUNARA') ? 2 : (country.id === 'DRAKMOOR' ? 1 : 0);
                ...
                if (requiredPortCount > 0) {
                    ...
                    if (coastalTris.length < requiredPortCount) {
                    ...
                    if (coastalTris.length >= requiredPortCount) {
                        if (country.id === 'LUNARA') {
                            ...
                            if (lunaraValid.length < requiredPortCount) lunaraValid = coastalTris;
                            
                            let mx = 480, my = 320;
                            lunaraValid.sort((a,b) => ( (a.center.x-mx)**2 + (a.center.y-my)**2 ) - ( (b.center.x-mx)**2 + (b.center.y-my)**2 ));
                            port1 = lunaraValid[0];
                            if (requiredPortCount > 1) {
                ...
                for (let i = 0; i < validTris.length; i++) {
                    ...
                    if (!tooClose) {
                        ...
                        // 25% chance for the city to span 2 connected triangles
                        let madeBig = false;
                        if (Math.random() > 0.75) {
                            let validNeighbor = candidate.neighbors.find(n => n.terrain === country.terrain);
                            if (validNeighbor) {
            ...
            edges.forEach(edge => {
                ...
                // Add microscopic deterministic noise to break ties consistently.
                // This forces all paths to prefer the same zig-zags, eliminating the DNA-like overlapping diamonds.
            ...
            countries.forEach(country => {
                ...
                // Calculate distance to nearest big city for each small city
                ...
                // Only connect half of the small cities (the ones closest to big cities) at the start of the game
                ...
                citiesToConnect.forEach(smallCity => {
                    ...
                    // Dijkstra from smallCity vertices to nearestBig vertices
                    ...
                    while(queue.length > 0) {
                        ...
                        neighbors.forEach(n => {
                            // Prevent autogenerated railroads from crossing into other countries
                            let inOtherCountry = n.edge.triangles.some(t => {
                                let tCountry = t.country || countries.find(c => c.terrain === t.terrain);
                                return tCountry && tCountry !== country;
                            });
                            
                            if (inOtherCountry) return;

                            // Apply a massive penalty to avoid routing through OTHER small cities
        ...
        function render() {
            ...
            triangles.forEach(tri => {
                ...
                if (tri.terrain === TERRAIN.CITY && tri.country) {
                    ctx.fillStyle = tri.country.cityColor;
                } else {
                    ctx.fillStyle = tri.terrain.color;
                }
                
                ctx.fill();
                if (tri.terrain !== TERRAIN.OCEAN) {
                    ctx.strokeStyle = 'rgba(0,0,0,0.03)'; // Subtle outlines for land
                    ctx.lineWidth = 1;
                    ctx.stroke();
                }
            });

            // 2. Draw Edges (Rivers & Railroads)
            edges.forEach(edge => {
                ...
                if (edge.isRiver && !isImpassable) {
                    ...
                    ctx.lineWidth = 2; // Slightly thicker so railroads show on top
                    ctx.stroke();
                }

                // Draw railroads or hover states ON TOP
                if (edge.hasRailroad) {
                    ctx.beginPath();
                    ctx.moveTo(edge.p1.x, edge.p1.y);
                    ctx.lineTo(edge.p2.x, edge.p2.y);
                    ctx.strokeStyle = edge.isRiver ? '#f1c40f' : '#ecf0f1'; // Bridge is yellow, railroad is white
                    ctx.lineWidth = 1.5;
                    ctx.stroke();
                    ...
                    ctx.moveTo(edge.p1.x, edge.p1.y);
                    ctx.lineTo(edge.p2.x, edge.p2.y);
                    ctx.strokeStyle = edge.isRiver ? '#f1c40f' : '#2ecc71'; // Preview color
                    ctx.lineWidth = 2;
                    ctx.stroke();
                }
            });
            ...
            triangles.forEach(tri => {
                if (tri.isPort) {
                    ...
                    if (tri.country && tri.country.id === 'DRAKMOOR') {
                        ctx.fillStyle = '#000080'; // Navy blue circle for Drakmoor
                    } else {
            ...
            regions.forEach(region => {
                // Find the coastal edge (triangle closest to the label)
                let closestTri = null;
                let minDist = Infinity;
                ...
                // Draw connecting line to the coast
                ctx.beginPath();
        ...
        canvas.addEventListener('click', (e) => {
            ...
            if (closest && !closest.hasRailroad) {
                ...
                let cost = 1;
                if (closest.isRiver) {
                    cost = 3; // Bridge cost 3x
                } else if (closest.triangles.some(t => t.terrain === TERRAIN.MOUNTAIN)) {
                    cost = 1.5; // Mountain railroad cost
                }

                totalCost += cost;
                const costDisplay = document.getElementById('cost-display');
                if (costDisplay) {
                    costDisplay.innerText = `$${totalCost}M`;
                }

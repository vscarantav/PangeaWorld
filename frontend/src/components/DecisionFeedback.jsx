...
export default function DecisionFeedback({ sessionId, round }) {
  ...
  return <details className="instructor-panel instructor-feedback"><summary><span className="instructor-summary-icon"><MessagesSquare aria-hidden="true" /></span><span><small>Review workspace</small><strong>Decision reasoning and round feedback</strong></span><em>{reviews.length} reviews</em></summary><div className="instructor-panel-body">
    {error && <p className="instructor-alert">{error}</p>}{!reviews.length && <p className="instructor-empty">Reviewed submissions will appear here.</p>}{reviews.map((item) => <article key={item.id}>
    ...
    </article>)}{reports.map((report) => <p className="instructor-intelligence" key={report.id}>Private intelligence · nation {report.target_id} · round {report.observed_round}: readiness {report.readiness}, posture {report.posture}. {report.limitation}</p>)}
  </div></details>;
}
